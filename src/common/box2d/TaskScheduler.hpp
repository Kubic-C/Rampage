...
namespace enki {
  ...
  public:
    ...
    // Execute range should be overloaded to process tasks. It will be called
    // with a range_ where range.start >= 0; range.start < range.end; and
    // range.end < m_SetSize; The range values should be mapped so that linearly
    // processing them in order is cache friendly i.e. neighbouring values
    // should be close together. threadnum_ should not be used for changing
    // processing of data, its intended purpose is to allow per-thread data
    // buckets for output.
    ...
    // Min Range - Minimum size of TaskSetPartition range when splitting a task
    ...
    // will *not* be a multiple of this, only attempts to deliver range sizes
    // larger than this most of the time. This should be set to a value which
    // results in computation effort of at least 10k clock cycles to minimize
    // task scheduler overhead. NOTE: The last partition will be smaller than
  ...
  struct ProfilerCallbacks {
    ...
    ProfilerCallbackFunc waitForTaskCompleteSuspendStart; // thread suspended waiting task
                                                          // completion
  ...
  ENKITS_API void* DefaultAllocFunc(size_t align_, size_t size_, void* userData_, const char* file_,
                                    int line_);
  ...
  struct TaskSchedulerConfig {
    // numTaskThreadsToCreate - Number of tasking threads the task scheduler
    // will create. Must be > 0. Defaults to GetNumHardwareThreads()-1 threads
    // as thread which calls initialize is thread 0.
    ...
    // thread used to initialize the TaskScheduler can also use the
    // TaskScheduler API. Thus there are (numTaskThreadsToCreate +
    // numExternalTaskThreads + 1) able to use the API, with this defaulting to
    // the number of hardware threads available to the system.
  ...
  public:
    ...
    // is sufficient to fill the system when including the main thread.
    ...
    // while( !GetIsShutdownRequested() ) {} can be used in tasks which loop, to
    // check if enkiTS has been requested to shutdown. If
    // GetIsShutdownRequested() returns true should then exit. Not required for
    // finite tasks Safe to use with WaitforAllAndShutdown() and ShutdownNow()
    // where this will be set Not safe to use with WaitforAll(), use
    // GetIsWaitforAllCalled() instead.
    inline bool GetIsShutdownRequested() const {
    ...
    // other Main thread should call this or use a wait to ensure its tasks are
    // run.
    ENKITS_API void RunPinnedTasks();

    // Runs the TaskSets in pipe until true == pTaskSet->GetIsComplete();
    // Should only be called from thread which created the task scheduler, or
    // within a task. If called with 0 it will try to run tasks, and return if
    // none are available. To run only a subset of tasks, set
    // priorityOfLowestToRun_ to a high priority. Default is lowest priority
    // available. Only wait for child tasks of the current task otherwise a
    // deadlock could occur. WaitforTask will exit if ShutdownNow() is called
    // even if pCompletable_ is not complete.
    ENKITS_API void WaitforTask(const ICompletable* pCompletable_,
                                enki::TaskPriority priorityOfLowestToRun_ = TaskPriority(TASK_PRIORITY_NUM -
                                                                                         1));

    // Waits for all task sets to complete - not guaranteed to work unless we
    // know we are in a situation where tasks aren't being continuously added.
    // If you are running tasks which loop, make sure to check
    // GetIsWaitforAllCalled() and exit WaitforAll will exit if ShutdownNow() is
    // called even if there are still tasks to run or currently running
    ...
    // This function can be safely called even if TaskScheduler::Initialize()
    // has not been called. This function will still wait for any running tasks
    // to exit before the task threads exit. ShutdownNow will cause tasks which
    // have been added to the scheduler but not completed to be in an undefined
    // state in which should not be re-launched.
    ...
    // Can be used with both ExternalTaskThreads or with an enkiTS tasking
    // thread to create a thread which only runs pinned tasks. If enkiTS threads
    // are used can create extra enkiTS task threads to handle non-blocking
    // computation via normal tasks.
    ...
    // and NO_THREAD_NUM for all other non-enkiTS threads which have not been
    // registered ( see RegisterExternalTaskThread() ), and <
    // GetNumTaskThreads() for all registered and internal enkiTS threads. It is
    // guaranteed that GetThreadNum() < GetNumTaskThreads() unless it is
    // NO_THREAD_NUM
    ENKITS_API uint32_t GetThreadNum() const;

    // Call on a thread to register the thread to use the TaskScheduling API.
    // This is implicitly done for the thread which initializes the
    // TaskScheduler Intended for developers who have threads who need to call
    // the TaskScheduler API Returns true if successful, false if not. Can only
    // have numExternalTaskThreads registered at any one time, which must be set
