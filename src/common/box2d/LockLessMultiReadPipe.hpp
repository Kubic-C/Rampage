...
namespace enki {
  // LockLessMultiReadPipe - Single writer, multiple reader thread safe pipe
  // using (semi) lockless programming Readers can only read from the back of
  // the pipe The single writer can write to the front of the pipe, and read
  // from both ends (a writer can be a reader) for many of the principles used
  // here, see
  // http://msdn.microsoft.com/en-us/library/windows/desktop/ee418650(v=vs.85).aspx
  // Note: using log2 sizes so we do not need to clamp (multi-operation) T is
  // the contained type Note this is not true lockless as the use of flags as a
  // form of lock state.
  ...
  inline bool LockLessMultiReadPipe<cSizeLog2, T>::ReaderTryReadBack(T* pOut) {
    ...
    // we update the read index using an atomic add, as we've only read one
    // piece of data. this ensure consistency of the read index, and the above
    // loop ensures readers only read from unread data
    ...
    T* ReaderReadBack() {
      ...
      if (pTailPlus1) {
        ...
        if (pTailPlus2) {
          ...
          tail.pNext = NULL;
          T* pCompare = pTailPlus1; // we need preserve pTailPlus1 as compare
                                    // will alter it on failure
