...
namespace enki {
  ...
  static constexpr int32_t gc_TaskAlmostCompleteCount = 1;
  // GetIsComplete() will return false, but execution is done and about to
  // complete
...
// For XCode before version 8 thread_local is not defined, so add to your
// compile defines: ENKI_THREAD_LOCAL __thread
...
// #elif __APPLE__
//         // Apple thread_local currently not implemented in XCode before
//         version 8 despite it being in Clang. #define ENKI_THREAD_LOCAL
//         __thread
#else
...
// each software thread gets its own copy of gtl_threadNum, so this is safe to
...
namespace enki {
  ...
  enum ThreadState : int32_t {
    ...
    ENKI_THREAD_STATE_PRIMARY_REGISTERED, // primary thread is the one enkiTS
                                          // was initialized on
  ...
  struct alignas(enki::gc_CacheLineSize) ThreadDataStore {
    ...
    char prevent_false_Share[enki::gc_CacheLineSize - sizeof(std::atomic<ThreadState>) -
                             sizeof(semaphoreid_t*) - sizeof(uint32_t)]; // required to prevent alignment
                                                                         // padding warning
  };
...
ENKITS_API void* enki::DefaultAllocFunc(size_t align_, size_t size_, void* userData_, const char* file_,
                                        int line_) {
  (void)userData_;
  (void)file_;
  (void)line_;
...
ENKITS_API void enki::DefaultFreeFunc(void* ptr_, size_t size_, void* userData_, const char* file_,
                                      int line_) {
  (void)size_;
  (void)userData_;
  (void)file_;
...
void TaskScheduler::StartThreads() {
  ...
  for (uint32_t thread = m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread();
  ...
  // first init as we want to be able to runtime change it
  if (1 == m_NumThreads) {
    m_NumPartitions = 1;
...
#ifdef ENKI_USE_WINDOWS_PROCESSOR_API
  // x64 bit Windows may support >64 logical processors using processor groups,
  // and only allocate threads to a default group. We need to detect this and
  // distribute threads accordingly
  if (GetNumHardwareThreads() > 64 && // only have processor groups if > 64 hardware threads
      std::thread::hardware_concurrency() < GetNumHardwareThreads() &&
      // if std::thread sees > 64 hardware threads no need to distribute
      std::thread::hardware_concurrency() < m_NumThreads)
  // no need to distribute if number of threads requested lower than std::thread
  // sees
  {
    ...
    if (success) {
      ...
      uint32_t currLogicalProcess = GetActiveProcessorCount((WORD)mainProcessorGroup);
      // we start iteration at end of current process group's threads

      // If more threads are created than there are logical processors then we
      // still want to distribute them evenly amongst groups so we iterate
      // continuously around the groups until we reach m_NumThreads
      uint32_t group = 0;
      while (currLogicalProcess < m_NumThreads) {
        ++group; // start at group 1 since we set currLogicalProcess to start of
                 // next group
        uint32_t currGroup = (group + mainProcessorGroup) % numProcessorGroups;
        // we start at mainProcessorGroup, go round in circles
        uint32_t groupNumLogicalProcessors = GetActiveProcessorCount((WORD)currGroup);
        ENKI_ASSERT(groupNumLogicalProcessors <= 64);
        uint64_t GROUPMASK = 0xFFFFFFFFFFFFFFFFULL >> (64 - groupNumLogicalProcessors);
        // group mask should not have 1's where there are no processors
        for (uint32_t groupLogicalProcess = 0;
          ...
          if (currLogicalProcess > m_Config.numExternalTaskThreads + GetNumFirstExternalTaskThread()) {
            ...
            // If a thread is assigned to a different group than the process,
            // the process's affinity is updated to include the thread's
            // affinity and the process becomes a multi-group process.
...
void TaskScheduler::StopThreads(bool bWait_) {
  // we set m_bWaitforAllCalled to true to ensure any task which loop using this
  // status exit
...
bool TaskScheduler::TryRunTask(uint32_t threadNum_, uint32_t priority_, uint32_t& hintPipeToCheck_io_) {
  ...
  if (!bHaveTask) {
    ...
    if (!bHaveTask) {
      // To prevent many threads checking the same task pipe for work we
      // pseudorandomly distribute the starting thread which we start checking
      // for tasks to run
  ...
  if (bHaveTask) {
    ...
    if (subTask.pTask->m_RangeToRun < partitionSize) {
      ...
      if (threadNum_ != threadToCheck) {
        ...
        // in order to ensure other threads can get enough work we need to split
        // into larger ranges these larger splits are then stolen and split
        // themselves otherwise other threads must keep stealing from this
        // thread, which may stall when pipe is full
        rangeToSplit = std::max(rangeToSplit,
                                (subTask.partition.end - subTask.partition.start) / gc_MaxStolenPartitions);
      }
...
void TaskScheduler::TaskComplete(ICompletable* pTask_, bool bWakeThreads_, uint32_t threadNum_) {
  ...
  // load of m_WaitingForTaskCount in this function, so we introduce a
  ...
  while (pDependent) {
    // access pTaskToRunOnCompletion member data before incrementing
    // m_DependenciesCompletedCount so they do not get deleted when another
    // thread completes the pTaskToRunOnCompletion
    int32_t dependenciesCount = pDependent->pTaskToRunOnCompletion->m_DependenciesCount;
    // get temp copy of pDependent so OnDependenciesComplete can delete task if
    // needed.
    ...
    if (dependenciesCount == (prevDeps + 1)) {
      // reset dependencies
      // only safe to access pDependentCurr here after above fetch_add because
      // this is the thread which calls OnDependenciesComplete after store with
      // memory_order_release
      pDependentCurr->pTaskToRunOnCompletion->m_DependenciesCompletedCount.store(0,
                                                                                 std::memory_order_release);
...
void TaskScheduler::WaitForTaskCompletion(const ICompletable* pCompletable_, uint32_t threadNum_) {
  ...
  ThreadState prevThreadState = m_pThreadDataStore[threadNum_].threadState.load(std::memory_order_relaxed);
  m_pThreadDataStore[threadNum_].threadState.store(ENKI_THREAD_STATE_WAIT_TASK_COMPLETION,
                                                   std::memory_order_seq_cst);
...
void TaskScheduler::SplitAndAddTask(uint32_t threadNum_, SubTaskSet subTask_, uint32_t rangeToSplit_) {
  ...
  int32_t upperBoundNumToAdd =
      2 + (int32_t)((subTask_.partition.end - subTask_.partition.start) / rangeToSplit_);
...
void TaskScheduler::AddTaskSetToPipeInt(ITaskSet* pTaskSet_, uint32_t threadNum_) {
  ...
  // Note: if m_SetSize is < m_RangeToRun this will be handled by SplitTask and
...
void TaskScheduler::InitDependencies(ICompletable* pCompletable_) {
  // go through any dependencies and set their running count so they show as not
  // complete and increment dependency count
...
void TaskScheduler::WaitForNewPinnedTasks() {
  ...
  uint32_t threadNum = gtl_threadNum;
  ThreadState prevThreadState = m_pThreadDataStore[threadNum].threadState.load(std::memory_order_relaxed);
  m_pThreadDataStore[threadNum].threadState.store(ENKI_THREAD_STATE_WAIT_NEW_PINNED_TASKS,
                                                  std::memory_order_seq_cst);
...
T* TaskScheduler::NewArray(size_t num_, const char* file_, int line_) {
  T* pRet = (T*)m_Config.customAllocator.alloc(alignof(T), num_ * sizeof(T),
                                               m_Config.customAllocator.userData, file_, line_);
...
T* TaskScheduler::Alloc(const char* file_, int line_) {
  T* pRet = (T*)m_Config.customAllocator.alloc(alignof(T), sizeof(T), m_Config.customAllocator.userData,
                                               file_, line_);
  return pRet;
}
...
TaskScheduler::TaskScheduler() :
    m_pPipesPerThread(), m_pPinnedTaskListPerThread(), m_NumThreads(0), m_pThreadDataStore(NULL),
    m_pThreads(NULL), m_bRunning(false), m_NumInternalTaskThreadsRunning(0),
    m_NumThreadsWaitingForNewTasks(0), m_NumThreadsWaitingForTaskCompletion(0), m_NumPartitions(0),
    m_pNewTaskSemaphore(NULL), m_pTaskCompleteSemaphore(NULL), m_NumInitialPartitions(0),
    m_bHaveThreads(false), m_NumExternalTaskThreadsRegistered(0) {}
...
Dependency::Dependency(const ICompletable* pDependencyTask_, ICompletable* pTaskToRunOnCompletion_) :
    pTaskToRunOnCompletion(pTaskToRunOnCompletion_), pDependencyTask(pDependencyTask_),
